...
The web app is unchanged. Linux and Windows still compile as a normal tray + window (see `cfg(target_os = "macos")` in `src-tauri/src/lib.rs` and `src-tauri/tauri.macos.conf.json`).
