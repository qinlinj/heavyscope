...
- **Dashboard + burn rate advisor** — four preset pools plus custom services, progress bars in each pool’s original accent (Heavy `#38bdf8`, Bot `#a78bfa`, Models `#34d399`, Other `#fbbf24`), remaining quota, reset countdown, and usage-tone on the used% text / RiskBadge only. Cursor Other is period USD (`planUsage.totalSpend` / `limit`, default $400) labeled Cursor period / spending — never `apiPercentUsed` and never a disabled onDemand $0. The advisor shows recommended daily pace, today used, waste / overspend risk, and cross-pool switch suggestions.
