...
- **Dashboard + burn rate advisor** — four preset pools plus custom services, progress bars in each pool’s original accent (Heavy `#38bdf8`, Bot `#a78bfa`, Models `#34d399`, Other `#fbbf24`), remaining quota, reset countdown, and usage-tone on the used% text / RiskBadge only. Cursor Other Models is `apiPercentUsed` as **0% used** (Included in Ultra / Other Models). `planUsage.totalSpend` / `limit` (live `$145.99 / $400`) is included / Auto, not Other. The advisor shows recommended daily pace, today used, waste / overspend risk, and cross-pool switch suggestions.
...
- **Tauri 2 tray** — system tray / macOS menu-bar shell around the same web UI and sql.js database. On macOS the shell is an **Accessory** (no Dock icon); the popup is a transparent window with **one ~400×660** panel (radius 12, no drop shadow) under the status item. `/tray` is the daily loop: sticky header (title, Refresh, Settings — no Day/Week/Month), a one-step **Go to Settings** line when neither token is pasted, a hero **used%** with an All / per-pool selector (All = tightest connected pool, labeled 全部 · name), a compact Advisor strip, hairline pool rows with each pool’s original accent bar, and a daily heatmap that can be drag-zoomed (min 2 week-columns, double-click reset, no hover popup). Browser `/tray` can finish that loop without a Mac. Layout Edit / Done is an icon; Done does not drop visible tiles. Linux/Windows keep a 980×720 tray window. Close hides to tray. Use Quit to exit. **Verify the menu-bar accessory on a real Mac** — see [docs/MACOS.md](docs/MACOS.md). UNVERIFIED on device.
...
- `preset-cursor-other` = Other Models from `planUsage.apiPercentUsed` (spending JS: `apiTitle="Other Models"`, `apiPercentage`). Stored as `quota_total=100`, `quota_used=apiPercentUsed`, unit `%`. Live **0% used**. `$400` is included cap copy, not used. `planUsage.totalSpend` / `limit` and disabled `onDemand.used` are **not** Other.
